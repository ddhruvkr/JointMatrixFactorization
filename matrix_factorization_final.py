...
def matrix_factorization(R, P, Q, K, steps=5000, alpha=0.003, beta=0.001):
    Q = Q.T
...
def matrix_factorization_implicit_semantic(G, R, S, T, P, Q, C, K, Cat, Tag, c, userMatrix, CNT, alpha = 0.003, steps=500, beta=0.001, gamma = 0.003):
    Q1 = Q
    Q = Q.T
    B = numpy.zeros(shape=(4))
    R = R / 4
                ...
                for j in userMatrix[i]:
                    if R[i][j] > 0:
                        ...
                        for k in range(K):
                            ...
                            if (P[i][k] < 0):
                                P[i][k] = 0
                            Q[k][j] = Q[k][j] + alpha * (2 * eij * P[i][k] - beta * P[i][k])
                            if (Q[k][j] < 0):
                                Q[k][j] = 0
...
def matrix_factorization_implicit_tag_time(G, R, S, T, P, Q, C, K, Cat, Tag, c, userMatrix, CNT, alpha = 0.003, steps=500, beta=0.001, gamma = 0.003):
    Q1 = Q
    Q = Q.T
    B = numpy.zeros(shape=(4))
    R = R / 4
                    ...
                    if R[i][j] > 0:
                        ...
                        if R[i][j] > 0.75 and R[i][j] <= 1:
                           div = B[3]
                            ...
                            div = B[0]
                        if div != 0:
                            eij = (R[i][j] - numpy.dot(P[i,:],Q[:,j]))/pow(div, 0.5) + (2 * gamma * val1)
                        else:
...
def matrix_factorization_implicit_semantic_tag_time(G, R, S, T, P, Q, C, K, Cat, Tag, c, userMatrix, CNT, alpha = 0.003, steps=500, beta=0.001, gamma = 0.003):
    Q1 = Q
    Q = Q.T
    B = numpy.zeros(shape=(4))
    R = R / 4
                    ...
                    if R[i][j] > 0:
                        ...
                        if R[i][j] > 0.75 and R[i][j] <= 1:
                           div = B[3]
                            ...
                            div = B[0]
                        if div != 0:
                            eij = (R[i][j] - numpy.dot(P[i,:],Q[:,j]))/pow(div, 0.5) + (2 * gamma * val) + (2 * gamma * val1)
                        else:
...
for ri in range(4):
    ...
    #for i in range(100):
    nP, nQ = matrix_factorization_implicit_tag_time(G, freq, S, T, P, Q, C, K, Cat, Tag, c, userMatrix, CNT, alpha)
    #lbd = lbd + 0.01
    #alpha = alpha + 0.0001
    ...
    for i in range(len(userMatrixTest)):
        for j in userMatrixTest[i]:
            cal_rating = numpy.dot(P[i][:], Q[:][j])
            
            if (cal_rating < 0):
                cal_rating = cal_rating * (-1)
            #print (str(freq[i][j]) + " " + str(cal_rating))
            error = error + pow((freq[i][j] / 4) - cal_rating, 2)
    #print (error)
